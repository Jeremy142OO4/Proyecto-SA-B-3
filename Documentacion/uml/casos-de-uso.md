...
| **Escenario Principal** | 1. El usuario selecciona **Nuevo Pago**, ingresa la cuenta origen, tipo de pago (`INTERNO` o `EXTERNO`), proveedor/servicio y monto. Para un pago externo selecciona el resultado simulado (`EXITO`, `FALLO` o `TIMEOUT`).<br>2. El sistema valida el formato de entrada y el API Gateway responde con `202 Accepted` conteniendo el `operationId`.<br>3. Payment Service registra el pago y el escenario solicitado en su base de datos con estado `PROCESANDO`.<br>4. Payment Service publica el comando de débito `cuenta.debito.solicitado` por RabbitMQ con `correlationId`.<br>5. Account Service valida fondos, realiza el débito y emite `cuenta.debitada`.<br>6. Si el pago es `EXTERNO`, Payment Service simula la respuesta del proveedor; `EXITO` conserva el débito y genera una referencia externa.<br>7. Payment Service actualiza el pago a `COMPLETADO`.<br>8. El servicio emite `pago.completado` y el resultado queda disponible para consulta. |
